...
- **`args.local_rank = int(os.environ.get("LOCAL_RANK", 0))`:**  위에서 말씀드린것처럼 `os.environ['LOCAL_RANK']'` 의 경우는 코드가 돌아가는 해당 GPU 번호를 의미하게 됩니다. 따라서, *os.environ* 내부에 있는 *get()* function을 통해  `args.local_rank`에 GPU 번호에 해당하는 변수를 받는 부분으로 이해해주시면 되겠습니다.
